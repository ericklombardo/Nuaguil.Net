...
namespace WCFChannelManager
{
        ...
        : AbstractProxyMethodBuilder
    {
        ...
        protected override void GenerateMethod(ILGenerator il, System.Reflection.MethodInfo method, System.Reflection.MethodInfo interfaceMethod)
        {
            ...
            // Parameter #3
            LocalBuilder typeParameters = il.DeclareLocal(typeof(Type[]));
            il.Emit(OpCodes.Ldc_I4, inParams.Count);
            il.Emit(OpCodes.Newarr, typeof(Type));
            il.Emit(OpCodes.Stloc, typeParameters);

            int paramIndex = 0;
            foreach (ParameterInfo paramInfo in inParams)
            {
                //Set the value for the current position for parameter #2
                ...
                //Set the value for the current position for parameter #3
                il.Emit(OpCodes.Ldloc, typeParameters);
                il.Emit(OpCodes.Ldc_I4, paramIndex);
                il.Emit(OpCodes.Ldtoken,paramInfo.ParameterType);
           ...
           if (interfaceMethod.ReturnType == typeof (void)) return;
